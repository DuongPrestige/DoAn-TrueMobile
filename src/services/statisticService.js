...
export const totalPriceDiscount = (price, discount) => {
        ...
        for (let i = 0; i < orderProduct.length; i++) {
          ...
          for (let j = 0; j < orderProduct[i].orderDetail.length; j++) {
            let receiptDetail = await db.ReceiptDetail.findAll({
              where: {
                productdetailconfigId: orderProduct[i].orderDetail[j].productId,
              },
              raw: true,
              nest: true,
            });
...
export const getStatisticStockProduct = (data) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let res = await db.ProductDetailConfig.findAndCountAll(objectFilter);
      for (let i = 0; i < res.rows.length; i++) {
        let receiptDetail = await db.ReceiptDetail.findAll({
          //
          where: { productdetailconfigId: res.rows[i].id },
        });
