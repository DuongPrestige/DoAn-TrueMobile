...
export const login = (data) => new Promise(async (resolve, reject) => {
    try {
        ...
        resolve({
            err: token ? 0 : 1,
            mes: token ? 'Login is successful' : response ? 'Password is wrong' : 'Email has not been registered',
            'access_token': token ? `Bearer ${token}` : token
        })
...
export const getOne = (usersId) => new Promise(async (resolve, reject) => {
    try {
...
export const getUserById = (userId) => new Promise(async (resolve, reject) => {
    try {
        ...
        const response = await db.User.findOne({
            ...
            raw: true,
            attributes: {
                exclude: ['password','firstName','lastName','address','genderId','phonenumber','dob','isActiveEmail','statusId','usertoken','createdAt','updatedAt']
            }
        })
...
export const deleteUser = (userId) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            resolve(response ? 
                (await response.destroy(), { err: 1, mes: 'Delete the user succeed' }) : 
