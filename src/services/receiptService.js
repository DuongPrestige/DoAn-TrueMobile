...
export const createNewReceipt = (data) => {
    return new Promise(async (resolve, reject) => {
        try {
            if (!data.userId || !data.supplierId || !data.productdetailconfigId || !data.quantity
                || !data.price
            ) {
                resolve({
                    errCode: 1,
...
export const createNewReceiptDetail = (data) => {
    return new Promise(async (resolve, reject) => {
        try {
            if (!data.receiptId || !data.productdetailconfigId || !data.quantity || !data.price)
             {
                resolve({
                    errCode: 1,
                    errMessage: 'Missing required parameter !'
    ...
    return new Promise(async (resolve, reject) => {
        try {
            if (!id) {
                ...
                res.receiptDetail = await db.ReceiptDetail.findAll({ where: { receiptId: id } , raw: true,
                            nest: true})
                ...
                if (res.receiptDetail && res.receiptDetail.length > 0) {
                    for (let i = 0; i < res.receiptDetail.length; i++) {
                        ...
                        res.receiptDetail[i].productDetailConfigData = productDetailConfig
                        res.receiptDetail[i].productDetailData = await db.ProductDetail.findOne({ where: { id: productDetailConfig.productdetailId } })
