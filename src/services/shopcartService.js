...
export const addShopCart = (data) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      } else if (!data.productdetailconfigId) {
        resolve({
          errCode: 1,
          errMessage: "Missing required productdetailconfigId !",
        });
      } else if (!data.quantity) {
        resolve({
          errCode: 1,
          errMessage: "Missing required quantity !",
        });
      } else {
        ...
        if (cart) {
          ...
          if (res) {
            let receiptDetail = await db.ReceiptDetail.findAll({
              where: { productdetailconfigId: res.id },
            });
